...
app.post('/contacts', upload.single('image'), (req, res) => {
    const { name, mobileNumbers } = req.body;
...
app.put('/contacts/:id', (req, res) => {
    ...
    const updateContactQuery = 'UPDATE contacts SET name = ? WHERE id = ?';
    db.query(updateContactQuery, [name, contactId], (err) => {
        if (err) {
            return res.status(500).send(err.message);
        }
...
app.get('/contacts/export/csv', (req, res) => {
    const fetchContactsQuery = `
      SELECT contacts.name, mobileNumbers.mobileNumber
      FROM contacts
      LEFT JOIN mobileNumbers ON contacts.id = mobileNumbers.contactId`;
